...
export const en = {
  ...
  'nav.health': 'Health',
  'memoryBackends.notInterchangeable': 'These two complement each other — they do not replace one another. Measured on real steps their recalls overlap by only 2–4%: mem0 answers semantically, Obsidian answers from role-scoped notes.',
  ...
  'memoryBackends.about.obsidian': 'Local vault. Role-scoped search over notes the agents wrote, newest first. Nothing leaves the box.',
  'memoryBackends.recalls': 'Recalls',
  'memoryBackends.emptyRecalls': 'Empty recalls',
  'memoryBackends.emptyHint': 'the honest signal — a full top-k means the cap was hit, not that k relevant things exist',
  'memoryBackends.stores': 'Writes',
  'memoryBackends.actors': 'Roles using it',
  'memoryBackends.lastActivity': 'Last activity:',
  'memoryBackends.neverUsed': 'No activity recorded in this window. Measured and idle — not unavailable.',
  ...
  'conversations.runsHint': 'Only runs whose agents actually spoke.',
  'conversations.messages': 'messages',
  'conversations.noRun': 'No run selected',
  'conversations.threadHint': 'What each role said, oldest first.',
  'conversations.emptyThread': 'This run recorded no agent output.',
  'conversations.replyReachesNextRun': 'This run has finished. A reply is filed against the role and reaches its NEXT run — it does not reach the agents above.',
  'conversations.role': 'Role',
  'conversations.reply': 'Reply',
  'conversations.replyPlaceholder': 'What should this role do differently?',
  ...
  'health.probeSurface.not_configured': 'agent surface: not configured',
  'health.probeSurface.ok': 'agent surface: {{backend}} responding',
  'health.probeSurface.unreachable': 'agent surface: {{backend}} NOT responding',
  'health.probeSurface.unknown_backend': 'agent surface: unknown backend configured',
  'health.probeSurface.unknown': 'agent surface: could not probe',
  'health.probeOtel.never_arrived': 'telemetry: none has ever arrived',
  'health.probeOtel.ok': 'telemetry: {{rows}} rows, newest {{hours}}h old',
  'health.probeOtel.stale': 'telemetry: STOPPED — newest row {{hours}}h old',
  'health.probeOtel.unknown': 'telemetry: age unknown',
  ...
  'efficiency.cacheTitle': 'prompt cache',
  'efficiency.cacheDescription': "Measured from our own per-step telemetry, not a tool's self-report. The headline rate is not the point — the list below is.",
  'efficiency.cacheNotMeasured': 'No model step has run yet — nothing to measure.',
  'efficiency.truncationTitle': 'context truncation',
  'efficiency.truncationDescription': 'Prior context that was cut before it reached a stage. This existed only as a log line until now: on run 639 `cap` mode kept the tail of the joined context, ~90% of the run vanished with both verdicts the release gate needed, and the gate then refused a release on a clearance that had been given. A week to find.',
  'efficiency.truncationNotMeasured': 'Not measured — this server does not report truncation, or the query failed.',
  'efficiency.truncationNoneRecorded': 'Nothing recorded. That is not the same as nothing truncated: it means no run has written one down yet.',
  'efficiency.truncationEvents': 'events',
  'efficiency.truncationDropped': 'characters dropped',
  'efficiency.truncationDroppedSub': 'across every recorded event',
  'efficiency.truncationWorstStage': 'largest single stage',
  'efficiency.truncationNoRole': 'no role recorded',
  'efficiency.truncationBasis.derived': 'budget derived from the model window',
  'efficiency.truncationBasis.fallback': 'budget from a configured fallback',
  'efficiency.truncationBasis.unknown': 'budget basis not recorded',
  'efficiency.cacheHitRate': 'hit rate',
  'efficiency.cacheSteps': '{count} model steps',
  'efficiency.cacheRead': 'cache read',
  'efficiency.cacheCreated': 'cache created',
  'efficiency.cacheUnamortisedTitle': 'Steps that create cache they never read back',
  'efficiency.cacheUnamortisedDescription': 'Median per run below 1.0, over enough turns that the prefix could have been read back on each of them and was not. Creation costs 1.25x base input, a read 0.1x — so this is full price, every run. Usually something variable sitting ahead of something stable in the prompt. Short steps are excluded: they sit below 1.0 by construction and reordering them fixes nothing.',
  'efficiency.cacheColStep': 'step',
  'efficiency.cacheColRuns': 'runs',
  'efficiency.cacheColCreated': 'created',
  'efficiency.cacheColRead': 'read back',
  'efficiency.cacheColTurns': 'turns',
  'efficiency.cacheColAmortisation': 'amortisation',
  'efficiency.cacheUnclassified': '{count} step(s) sit below the floor with no turn count recorded, so they cannot be judged either way — every run predating turn capture. They are not in the table above, and their absence is not a clean bill of health.',
  'efficiency.proxyTitle': 'compression proxy',
  'efficiency.proxyDescription':
    'On the request path of every agent call. It falls back to a direct call when unreachable, which keeps agents working and silently stops compressing them — this panel is where that shows.',
  'efficiency.proxyNotAvailable': 'not answering — unconfigured, or down. Agents are dispatching directly.',
  'efficiency.proxyRequests': 'Requests',
  'efficiency.proxyMode': 'mode {mode}',
  'efficiency.proxyCompressed': 'Compressed',
  'efficiency.proxyTokensRemoved': 'Tokens removed',
  'efficiency.proxySaved': 'Saved',
  'efficiency.proxyAgainst': 'against {total}',
