...
export const fr: Record<TranslationKey, string> = {
  ...
  'nav.health': 'Santé',
  'memoryBackends.notInterchangeable': 'Les deux se compl\u00e8tent, ils ne se remplacent pas. Mesur\u00e9 sur des \u00e9tapes r\u00e9elles, leurs rappels ne se recoupent qu\'\u00e0 2\u20134 % : mem0 r\u00e9pond s\u00e9mantiquement, Obsidian r\u00e9pond depuis les notes cloisonn\u00e9es par r\u00f4le.',
  ...
  'memoryBackends.about.obsidian': 'Vault local. Recherche cloisonn\u00e9e par r\u00f4le sur les notes \u00e9crites par les agents, du plus r\u00e9cent au plus ancien. Rien ne sort.',
  'memoryBackends.recalls': 'Rappels',
  'memoryBackends.emptyRecalls': 'Rappels vides',
  'memoryBackends.emptyHint': 'le signal honn\u00eate \u2014 un top-k plein signifie que la limite est atteinte, pas que k choses pertinentes existent',
  'memoryBackends.stores': '\u00c9critures',
  'memoryBackends.actors': 'R\u00f4les utilisateurs',
  'memoryBackends.lastActivity': 'Derni\u00e8re activit\u00e9 :',
  'memoryBackends.neverUsed': 'Aucune activit\u00e9 sur la p\u00e9riode. Mesur\u00e9 et inactif \u2014 pas indisponible.',
  'memory.tabBackends': 'Sources',
  'nav.cache': 'Cache',
  'nav.plugins': 'Plugins',
  ...
  'conversations.runs': 'Runs',
  'conversations.runsHint': 'Seulement les runs dont les agents se sont exprimés.',
  'conversations.messages': 'messages',
  'conversations.noRun': 'Aucun run sélectionné',
  'conversations.threadHint': 'Ce que chaque rôle a dit, du plus ancien au plus récent.',
  'conversations.emptyThread': 'Ce run n’a enregistré aucune sortie d’agent.',
  'conversations.replyReachesNextRun': 'Ce run est terminé. Une réponse est consignée pour le rôle et atteint son PROCHAIN run — elle n’atteint pas les agents ci-dessus.',
  'conversations.role': 'Rôle',
  'conversations.reply': 'Réponse',
  'conversations.replyPlaceholder': 'Que ce rôle devrait-il faire différemment ?',
  ...
  'agents.binaries.colAgent': 'Agent',
  'agents.binaries.colVersion': 'Installé',
  ...
  'agents.binaries.update': 'Mettre à jour',
  'agents.binaries.confirmUpdate': 'Confirmer la mise à jour',
  'agents.binaries.cancel': 'Annuler',
  'agents.binaries.working': 'En cours…',
  'agents.binaries.failed': 'échec — voir les logs serveur',
  'agents.binaries.docsOnly': 'docs uniquement (pas de one-liner vérifié)',
  'agents.binaries.adminOnly': 'Rôle admin requis',
  ...
  'health.probeSurface.not_configured': 'surface agent : non configurée',
  'health.probeSurface.ok': 'surface agent : {{backend}} répond',
  'health.probeSurface.unreachable': 'surface agent : {{backend}} NE répond PAS',
  'health.probeSurface.unknown_backend': 'surface agent : backend inconnu configuré',
  'health.probeSurface.unknown': 'surface agent : sonde impossible',
  'health.probeOtel.never_arrived': "télémétrie : rien n'est jamais arrivé",
  'health.probeOtel.ok': 'télémétrie : {{rows}} lignes, la plus récente à {{hours}} h',
  'health.probeOtel.stale': 'télémétrie : ARRÊTÉE — ligne la plus récente à {{hours}} h',
  'health.probeOtel.unknown': 'télémétrie : âge inconnu',
  'health.title': 'Plugins',
  'health.description':
    "État global des plugins, identique à `hivepilot plugins health`. Trié du pire au meilleur.",
  'health.verdict.checking': 'Vérification…',
  'health.verdict.ok': 'Tout est opérationnel',
  'health.verdict.attention': "{count} point(s) d'attention",
  'health.verdict.broken': '{count} en panne',
  'health.verdict.reasonDb': 'erreur base de données',
  'health.verdict.reasonPlugin': '{name} en erreur',
  'health.verdict.reasonNeverRan': '{name} ok mais jamais exécuté',
  'health.verdict.reasonSurface': 'surface agent injoignable',
  'health.verdict.reasonOtel': 'télémétrie arrêtée',
  'health.verdict.reasonUnverifiable': '{name} non vérifiable',
  'health.service.title': 'Service',
  'health.service.description':
    'Le processus API lui-même — base de données, runners, dépendances optionnelles.',
  'health.service.database': 'Base de données',
  'health.service.runners': 'Runners',
  'health.service.deps': 'Dép. optionnelles',
  'health.service.depMissingTitle':
    'Non installée — un choix, pas une erreur. À installer seulement si un plugin en a besoin.',
  'health.service.unreachable': "Le point de santé du service n'a pas pu être lu.",
  'health.probes.title': 'Sondes système',
  ...
  'efficiency.truncationTitle': 'troncature de contexte',
  'efficiency.truncationDescription': "Contexte antérieur coupé avant d'atteindre une étape. Cela n'existait que sous forme de ligne de log : au run 639, le mode `cap` a gardé la fin du contexte joint, ~90 % du run a disparu avec les deux verdicts dont le gate avait besoin, et le gate a refusé une release sur une autorisation qui avait été donnée. Une semaine pour le trouver.",
  'efficiency.truncationNotMeasured': 'Non mesuré — ce serveur ne rapporte pas la troncature, ou la requête a échoué.',
  'efficiency.truncationNoneRecorded': "Rien d'enregistré. Ce n'est pas la même chose que rien de tronqué : aucun run ne l'a encore écrit.",
  'efficiency.truncationEvents': 'événements',
  'efficiency.truncationDropped': 'caractères perdus',
  'efficiency.truncationDroppedSub': 'sur tous les événements enregistrés',
  'efficiency.truncationWorstStage': 'plus grande étape seule',
  'efficiency.truncationNoRole': 'aucun rôle enregistré',
  'efficiency.truncationBasis.derived': 'budget dérivé de la fenêtre du modèle',
  'efficiency.truncationBasis.fallback': 'budget issu du repli configuré',
  'efficiency.truncationBasis.unknown': 'base du budget non enregistrée',
  ...
  'efficiency.cacheUnamortisedDescription': "Médiane par run sous 1,0, sur assez de tours pour que le préfixe ait pu être relu à chacun — et il ne l'a pas été. Une création coûte 1,25x l'entrée, une lecture 0,1x — donc plein tarif, à chaque run. Généralement du variable placé avant du stable dans le prompt. Les étapes courtes sont exclues : elles passent sous 1,0 par construction, et les réordonner ne corrige rien.",
  'efficiency.cacheColStep': 'étape',
  'efficiency.cacheColRuns': 'runs',
  'efficiency.cacheColCreated': 'créé',
  'efficiency.cacheColRead': 'relu',
  'efficiency.cacheColTurns': 'tours',
  'efficiency.cacheColAmortisation': 'amortissement',
  'efficiency.cacheUnclassified': "{count} étape(s) passent sous le seuil sans nombre de tours enregistré : impossible de trancher dans un sens ou dans l'autre — tous les runs antérieurs à la capture des tours. Elles ne figurent pas dans le tableau ci-dessus, et leur absence n'est pas un satisfecit.",
