...
| State store | SQLite `state.db` holding runs, steps, interactions, verdicts, lessons, drift scans, tenants, the HP-97 PASS inbox (`pass_proposals`), and HP-99 tenant-scoped evidence refs (`evidence_refs`, watermark = `change_log.id`); per-run `runs/<timestamp>/summary.json`; structured JSON logs. |
