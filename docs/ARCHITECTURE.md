...
| State store | SQLite `state.db` holding runs, steps, interactions, verdicts, lessons, drift scans, tenants, and the HP-97 PASS inbox (`pass_proposals`); per-run `runs/<timestamp>/summary.json`; structured JSON logs. |
