...
| State store | SQLite `state.db` holding runs, steps, interactions, verdicts, lessons, drift scans, tenants, the HP-97 PASS inbox (`pass_proposals`), HP-99 tenant-scoped evidence refs (`evidence_refs`, watermark = `change_log.id`), HP-100 `side_effects` / `pending_tool` checkpoints, and HP-104 skill-cycle events (`skill_cycle_events`, idempotent per revision/run/step/type); per-run `runs/<timestamp>/summary.json`; structured JSON logs. |
