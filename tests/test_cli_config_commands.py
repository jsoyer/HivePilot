...
class TestRoleWire:
    ...
    def test_runner_valid_kind(self, config_dir: Path) -> None:
        ...
        # PATH. Use "openrouter" instead: a kind that stays unconditionally
        ...
        result = runner.invoke(app, ["role", "wire", "developer", "runner", "openrouter"])
        assert result.exit_code == 0, result.output
        assert "runner: openrouter" in (config_dir / "roles.yaml").read_text()
