...
class TestVaultSecurityPath:
    ...
    def test_returns_path_when_vault_is_absolute(self):
        ...
        # "08 - Security" directly (no extra "Noxys" subdirectory — the vault path
        # is already scoped to the product, e.g. .../obsidian-vault/Noxys).
...
class TestNoxysRootBackwardCompat:
    """_NOXYS_ROOT module-level variable must be a string (backward-compat shim)."""

    def test_noxys_root_is_a_string(self):
        assert isinstance(agent_rules._NOXYS_ROOT, str)

    def test_noxys_root_matches_settings_governance_repo(self):
        expected = settings.governance_repo or ""
        assert agent_rules._NOXYS_ROOT == expected
