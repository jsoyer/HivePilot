"""Minimal companion test file for `plugins/pi.py`.
...
`register()`'s gating for `pi` in isolation so it isn't a no-op stub.
"""
...
def _load_plugin_module():
    path = REPO_ROOT / "plugins" / "pi.py"
    spec = importlib.util.spec_from_file_location("hivepilot_plugin_pi_smoke", path)
...
def test_register_returns_pi_runner_when_active(monkeypatch) -> None:
    module = _load_plugin_module()
    monkeypatch.setattr(settings, "pi_enabled", True, raising=False)
    with patch.object(module.shutil, "which", return_value="/usr/local/bin/pi"):
        hooks = module.register()
    assert hooks.get("runners") == {"pi": PiRunner}
