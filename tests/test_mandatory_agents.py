...
Covers `hivepilot init` (hard-fail when none of claude/codex/vibe are on
PATH; warning-but-continue when claude specifically is missing) and
`hivepilot doctor` (mandatory-agent verdict + optional OPENROUTER_API_KEY
note).
...
def test_init_exits_nonzero_when_no_mandatory_agent_present(
