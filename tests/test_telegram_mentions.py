...
def _make_tables(extra_groups=None, extra_projects=None):
    ...
    groups = {"acme": type("G", (), {"hub": "acme", "components": ["acme-api", "acme-ui"]})()}
    if extra_groups:
        groups.update(extra_groups)
    agents_known = set(_ALIAS_TO_ROLE.keys())
    projects_known = {"acme", "acme-api", "acme-ui"}
    if extra_projects:
...
class TestParseMention:
    ...
    def test_non_at_text_returns_none(self):
        ...
        kind, name, rest = _parse_mention(
            "hello world", groups=groups, agents_known=agents, projects_known=projects
        )
        assert kind == "none"
    ...
    def test_empty_string_returns_none(self):
        groups, agents, projects = _make_tables()
        kind, name, rest = _parse_mention(
            "", groups=groups, agents_known=agents, projects_known=projects
        )
        assert kind == "none"
    ...
    def test_agent_mention_gustave(self):
        groups, agents, projects = _make_tables()
        kind, name, rest = _parse_mention(
            "@gustave fix the auth bug", groups=groups, agents_known=agents, projects_known=projects
        )
        assert kind == "agent"
    ...
    def test_agent_mention_blaise_with_target(self):
        """@blaise @acme-api review this — target stays in rest for _cmd_mention to parse."""
        groups, agents, projects = _make_tables()
        kind, name, rest = _parse_mention(
            "@blaise @acme-api review this",
            groups=groups,
            agents_known=agents,
            projects_known=projects,
        )
        assert kind == "agent"
        assert name == "cto"
        assert rest == "@acme-api review this"

    def test_group_mention(self):
        groups, agents, projects = _make_tables()
        kind, name, rest = _parse_mention(
            "@acme ship the device-fleet API",
            groups=groups,
            agents_known=agents,
            projects_known=projects,
        )
    ...
    def test_group_wins_over_project_collision(self):
        """When 'acme' is both a group and a project name, group wins."""
        groups, agents, projects = _make_tables()
        kind, name, rest = _parse_mention(
            "@acme do something", groups=groups, agents_known=agents, projects_known=projects
        )
        assert kind == "group"

    def test_project_mention_when_not_a_group(self):
        """acme-api is a project but not a group — should resolve to project."""
        groups, agents, projects = _make_tables()
        kind, name, rest = _parse_mention(
            "@acme-api review the endpoints",
            groups=groups,
            agents_known=agents,
            projects_known=projects,
        )
    ...
    def test_unknown_token(self):
        groups, agents, projects = _make_tables()
        kind, name, rest = _parse_mention(
            "@unknownbot do stuff", groups=groups, agents_known=agents, projects_known=projects
        )
        assert kind == "unknown"
    ...
    def test_accent_normalization(self):
        ...
        groups, agents, projects = _make_tables()
        kind, name, rest = _parse_mention(
            "@alienor review this", groups=groups, agents_known=agents, projects_known=projects
        )
        assert kind == "agent"
        assert name == "ceo"
    ...
    def test_case_insensitive(self):
        groups, agents, projects = _make_tables()
        kind, name, rest = _parse_mention(
            "@GUSTAVE fix tests", groups=groups, agents_known=agents, projects_known=projects
        )
        assert kind == "agent"
        assert name == "developer"
    ...
    def test_rest_stripped(self):
        groups, agents, projects = _make_tables()
        kind, name, rest = _parse_mention(
            "@gustave   fix bug  ", groups=groups, agents_known=agents, projects_known=projects
        )
        assert rest == "fix bug"
    ...
    def test_mention_only_no_rest(self):
        ...
        groups, agents, projects = _make_tables()
        kind, name, rest = _parse_mention(
            "@gustave", groups=groups, agents_known=agents, projects_known=projects
        )
        assert kind == "agent"
        assert rest == ""
    ...
    def test_all_agent_aliases_resolve(self):
        ...
        for alias in _ALIAS_TO_ROLE:
            kind, name, rest = _parse_mention(
                f"@{alias} do it", groups=groups, agents_known=agents, projects_known=projects
            )
