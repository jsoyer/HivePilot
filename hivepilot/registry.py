...
_BUILTIN_RUNNERS: Dict[str, Type[BaseRunner]] = {
    ...
    # HP-18: OpenAI-compatible endpoints (OpenAI, OpenCode Zen/Go, Ollama
    # Cloud, vLLM, …) via `OPENAI_BASE_URL` + `OPENAI_API_KEY` — API-only,
    # no CLI binary, same rationale as `openrouter` above. Lets the concierge
    # classifier run on a hosted OSS model (see concierge_service.py).
