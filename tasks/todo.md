...
- The vault **IS git-versioned**: repo root is the PARENT `~/Documents/Github/jsoyer/obsidian-vault/.git`, remote `git@github.com:jsoyer/obsidian-vault.git`, branch `main`, clean + synced. `Noxys/` is a subfolder. → No `git init`; use a branch.
- The vault is an **org-wide source of truth referenced by absolute path** from the noxys monorepo: `08 - Security` (rules, specs, code, INVARIANTS), `03 - Decisions` (sync-adrs.sh, ML readme), `02 - Architecture` (ML readme), `01 - Journal` (agent pushes), and the whole repo (noxys-product-hub CI). → **Renaming/renumbering breaks cross-repo refs and is NOT git-reversible.**
- `sales-questionnaires` is the SoT mirrored to `noxys-eu/noxys-sales-questionnaires` for external sharing. → **Leave embedded; do NOT de-embed.**

## Locked decisions

- **Vault:** `…/obsidian-vault/Noxys` (git-versioned via parent repo, remote, on `main`).
...
- **Rules wiring:** reference-by-path (no copy); artifacts in **English** (noxys `.cursorrules`).
