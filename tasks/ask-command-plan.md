...
- `[@cible]` optionnelle = projet ou groupe. Absente → cible par défaut
  (`HIVEPILOT_DEFAULT_TARGET`, défaut `acme`). Si la cible est un **groupe**,
  l'ordre tourne sur le **hub** (les rôles planif s'y prêtent ; pour un ordre de
  code, préciser `@<composant>`, ex. `@acme-api`).
...
- [ ] `config.py` : `default_target: str = "acme"` (`HIVEPILOT_DEFAULT_TARGET`).
