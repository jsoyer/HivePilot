...
entamé : `acme-developer` runs = **3 success / 0 deferred / 13 failed** (les 13 = ancien
crash quota d'avant l'auto-resume → marqués `failed`, non repris). Seul **acme-api** a du
vrai code (branches `feat/siem-tls-realtime-emission` + `hivepilot/acme-api`, non mergées).
...
- [ ] Reviewer (Victor) → ouvrir les PRs → merger le SIEM acme-api (humain merge).
- [ ] Finir les ~12 composants restants.
- [ ] Purger/relancer les 13 runs `failed`.

## 2. Distribution — génériciser HivePilot (ta cible)
Rendre l'outil générique/livrable.
- [ ] `_ACME_ROOT = "/home/jeromesoyer/Documents/Github/acme"` **hardcodé** (`agent_rules.py`,
...
- [ ] Le groupe `acme`, les noms d'agents FR, les `ACME_*` constants → templates de config
...
- [ ] Séparer "moteur HivePilot" (code) de "config produit" (acme = un exemple).

## 3. Nettoyage / hygiène
- [ ] Ajouter `.hivepilot-wt/` au `.gitignore` des repos composants (worktrees jetables).
- [ ] Sort des branches `hivepilot/acme-api` + `feat/siem-*` (merger ou supprimer).
